...
{
    /// <summary>
    ///     Keeps a prometheus scrape job in sync with the windows nodes to get the windows node exporter metrics pulled into
    ///     prometheus
    /// </summary>
    internal class SyncService : IHostedService, IDisposable
    {
        ...
        private async Task SyncWithWindowsNodesAsync(CancellationToken cancellationToken)
        {
            ...
            _logger.LogDebug(
                $"Discovered windows nodes: {Environment.NewLine + "\t" + string.Join(Environment.NewLine + "\t", windowsNodes)}");

            var prometheusServerConfigMap =
                await _kubernetesClientWrapper.GetPrometheusScrapeConfigSecretAsync(cancellationToken);

            var tracker =
                new PrometheusConfigChangeTracker(
                    Encoding.UTF8.GetString(prometheusServerConfigMap.Data[SecretDataPropertyName]));
            ...
            foreach (var node in windowsNodes.Where(x => !windowsNodeScrapeConfigs.Contains(x))) tracker.Add(node);
